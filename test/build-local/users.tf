...
locals {
  # NOTE: using bcrypt causes this to be regenerated (with different salt) each time
  # terraform is applied. See the terraform docs for details.
  passwords = { for name, user in var.users: name => bcrypt(user.password) }
